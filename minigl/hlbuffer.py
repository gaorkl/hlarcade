...
class HLBuffer:
    ...
    def __init__(
        self, ctx: "HLContext", data: Optional[Any] = None, reserve: int = 0, usage: str = "static"
    ):

        self._ctx = ctx
        self._glo = glo = gl.GLuint()
        self._size = -1
        self._usage = HLBuffer._usages[usage]
        ...
        if self._ctx.gc_mode == "auto":
            weakref.finalize(self, HLBuffer.delete_glo, self.ctx, glo)
    ...
    def delete(self):
        ...
        """
        HLBuffer.delete_glo(self._ctx, self._glo)
        self._glo.value = 0

    @staticmethod
    def delete_glo(ctx: "HLContext", glo: gl.GLuint):
        """
    ...
    def copy_from_buffer(self, source: "HLBuffer", size=-1, offset=0, source_offset=0):
